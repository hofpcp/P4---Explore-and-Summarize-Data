...
# Loading the Data and changing to ordered factors where necessary and limiting dataset
# to only data post July 2009. As well as renaming variables to start with lowercase. 
...
loansnew <- loans[, c('Term', 'LoanStatus', 'BorrowerRate', 'EstimatedLoss',
                      ...
                      'CreditScoreRangeUpper', 'IncomeRange', 'LoanOriginalAmount')]
...
loansnew <- plyr::rename(loansnew, c('Term' = 'term', 'LoanStatus' = 'loanStatus',
                                     'BorrowerRate' = 'borrowerRate', 'EstimatedLoss' =
                                       'estimatedLoss',  'ProsperRating..Alpha.' =
                                       'prosperRating', 'ProsperScore' = 'prosperScore',
                                     'ListingCategory..numeric.' = 'listingCategory',
                                     'EmploymentStatus' = 'employmentStatus',
                                     'IsBorrowerHomeowner' = 'isBorrowerHomeowner',
                                     'IncomeRange' = 'incomeRange', 'LoanOriginalAmount' =
...
# Cutting LoanStatus data so that "PasDue...."" buckets are including in one bucket.
...
ggplot(aes(x = loanOriginalAmount), data = loansnew) +
  ...
  ggtitle('Distribution of LoanOriginalAmount')
...
# Histogram CreditScoreRangeAvg and extra histogram with adjusted bin size and adjusted
# x-axis
...
# In order to make correlation martix and variable pairs overview, numeric variables
# are collected in data subset
...
ggplot(aes(x = borrowerRate, y = prosperRating), data = loansnew) +
  geom_point() +
...
# faceting can be done more clearly/specified


loansnewmulti <- loansnew[, c(3,4,5,6,12)] 

loansnewmulti$prosperScore <- cut(loansnewmulti$prosperScore, br = 1:11)                      
...
ggplot(aes(x= creditScoreRangeAvg, y = borrowerRate), data = loansnewmulti) +
  geom_point(aes(color = prosperScore), size = 2) +
  scale_fill_brewer(type = 'qual') +
...
ggplot(aes(x = creditScoreRangeAvg, y = estimatedLoss), data = loansnew) +
  ...
  ggtitle('Mean EstimatedLoss vs. CreditScoreRangeAvg specified on ProsperRating')

ggplot(aes(x = creditScoreRangeAvg, y = estimatedLoss), data = loansnewmulti) +
  geom_line(aes(color = prosperScore), stat = 'summary', fun.y = mean )+
  ggtitle('Mean EstimatedLoss vs. CreditScoreRangeAvg specified on ProsperScore')
...
ggplot(aes(x = creditScoreRangeAvg, y = borrowerRate), data = loansnewmulti) +
  ...
  ggtitle('BorrowerRate vs. CreditScoreRangeAvg faceted via ProsperScore')

ggplot(aes(x = creditScoreRangeAvg, y = estimatedLoss), data = loansnewmulti) +
  facet_wrap(~ prosperRating) +
  geom_jitter(alpha = 1/4 , aes(color = prosperScore)) +
  scale_fill_brewer(type = 'div') +
  ggtitle('EstimatedLoss vs. CreditScoreRangeAvg faceted via ProsperScore')

ggplot(aes(x = creditScoreRangeAvg, y = estimatedLoss/borrowerRate), data = loansnewmulti) +
  facet_wrap(~ prosperRating) +
  geom_jitter(alpha = 1/4 , aes(color = prosperScore)) +
  scale_fill_brewer(type = 'div') +
  ggtitle('EstimatedLoss/BorrowerRate vs. CreditScoreRangeAvg faceted via ProsperScore')
...
BorrowerRates is between normal and Bi-modal distribution. Faceting this with ProsperRating we can see that the BorrowerRate peak > 0.3 is due to a heavy count of loans with a "HR" prosperRating (HR = High Risk)  
...
Building further on analysis and insights from univariate and bi-variate analysis, we arrive at a decent illustration hightlighting the consistency across records and the chosen variables, which is expected in financial loan records. I.e. a very clear relationship showing that the lower the CreditRatingScoreAvg/ProsperScore/ProsperRating the higher the estimatedLoss/borrowerRatio. Also as ProsperRating/creditScoreRangeAvg improves (rightshift in charts and data distribution) the color changes towards better prosperScore and lower ratio of estimatedLoss/borrowerRate.   
